                    ...
                    <div class="col-sm-9">
                        <div class="input-daterange input-group">
                            <input type="text" class="form-control salesman month" id="month" style="padding-left: 20px"
                                   value="{{date('M-Y')}}" name="month"/>
                        </div>
                        <div id="dataByMonth"></div>
                    </div>

                    <div class="col-sm-1">
                        <div class="btn-group pull-right">
                            <a href="{{route('salary.index')}}" class="btn btn-success btn-block"><i
                                    class="zmdi zmdi-money"> </i> वेतन generate करें
                            </a>
                        </div>
                    </div>
    ...
    <script>
        $(function () {
            $('#datatables').DataTable({
                ...
                stateSave: true,
                ajax: {
                    url: '{{ route('generatedSalary.getData') }}',
                    data: function (d) {
                        d.month = $('#month').val();
                    }
                },
    ...
    <script>
        function PayIt(id) {
            ...
            swal({
                ...
                confirmButtonText: "हाँ, इसे Payment करे!",
                CancelButtonText: "नहीं",
            }).then(function (isConfirm) {
                if (isConfirm) {
                    $.ajax({
                        url: '{{ url('admin/generated-salary') }}/' + id,
                        type: 'post',
                    ...
                    swal("Done!", "Payment दिया गया है", "success");
                }
            });
        }
    </script>
